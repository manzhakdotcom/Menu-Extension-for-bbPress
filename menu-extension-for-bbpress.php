...
Plugin URI: https://github.com/manzhakdotcom/Menu-Extension-for-bbPress
...
define( 'BBP_M_EXT_DIR', plugin_dir_path( __FILE__ ) );
define( 'BBP_M_EXT_INC_DIR', trailingslashit( BBP_M_EXT_DIR . 'includes'  ) );
...
function bbp_m_ext_start() {
	...
	if ( bbp_m_ext_is_request( 'admin' ) ) {
        require( BBP_M_EXT_INC_DIR . 'admin.php' );
	}
	if ( bbp_m_ext_is_request( 'frontend' ) ) {
        require( BBP_M_EXT_INC_DIR . 'frontend.php' );
	}

}


function bbp_m_ext_is_bbpress_active() {
    return get_option('active_plugins') > '' && in_array( 'bbpress/bbpress.php', get_option('active_plugins') );
}

if ( bbp_m_ext_is_bbpress_active() ) {
